...
public class ElectronicStoreController {
    ...
    private List<Item> items = new ArrayList<>(Arrays.asList(new Item(String.valueOf(serialItemId++), "Apple iPhone 12 Pro Max", "Smartphone", 1490000, "A14 Bionic", "512GB"),
    ...
    public Item findItemByPathId(@PathVariable String id){
        Item itemFounded = items.stream().filter(item -> item.getId().equals(id)).findFirst().orElseThrow(() ->new RuntimeException());
        return itemFounded;
    }
    ...
    public String deleteItemByPathId(@PathVariable String id){
     ...
     return "Object with id = " + itemFounded.getId() + " has been removed";
    }
